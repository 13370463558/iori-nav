...
export async function onRequest(context) {
  ...
  const uuidFooterIcon = `
    <button id="generateUuidBtn" type="button" class="footer-icon-btn" title="生成 UUID 并复制" aria-label="生成 UUID 并复制到剪切板">
      <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M4 4h6v6H4zM14 4h6v6h-6zM4 14h6v6H4z"/><path d="M14 14h2v2h-2zM18 14h2v2h-2zM14 18h2v2h-2zM18 18h2v2h-2z"/></svg>
    </button>`;
  ...
  // footer 按钮顺序：模式切换 → Github → UUID → 管理
  const footerButtonsHtml = `${themeIconHtml}${githubFooterIcon}${uuidFooterIcon}${S.home_hide_admin ? '' : adminFooterIcon}`;
